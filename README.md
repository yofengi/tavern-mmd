...
| 能力 | 说明 |
|---|---|
| 三平台支持 | 当前 MMD（支持 `<script>`）/ 旧版 MMD（不支持）/ 本地酒馆 SillyTavern |
...
| 指令 | 用途 |
|---|---|
| `/cardplan` | 角色卡标准流程：弹窗快问快答 → plan.md → 执行 |
| `/cardplanmax` | 角色卡深度共创：开放讨论 → 方案收敛 → 分节设计确认 → 分段执行 |
...
- 旧版 MMD 平台限制与架构模式整理自社区实测的避坑指南与 KV-Robust V4.0 状态栏规范
