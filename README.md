...
skill 自带两个纯 Python 标准库脚本（零依赖，全 agent 通用），用于交付前质量把关：

- `scripts/validate.py` — 静态审核：JSON 合法性、BOM、双重转义、平台红线（script/ES6/innerHTML）、v2 规范、世界书字段。子代理可调用以节约上下文。
- `scripts/build-preview.py` — 平台保真预览：生成自包含 HTML 沙箱，按平台模拟渲染限制（旧版 MMD 剥离 `<script>` 并裸露源码、ES6 标黄）。主AI 用 Preview 工具看渲染、测交互。

用法见 `skills/tavern-mmd/scripts/README.md`。
