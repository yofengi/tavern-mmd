...
skill 自带两个纯 Python 标准库脚本（零依赖，全 agent 通用），用于交付前质量把关。用法详见 `skills/tavern-mmd/scripts/README.md`。

### JSON 格式审核（`scripts/validate.py`）

MMD 导入 json 的格式错误是导入失败的头号原因，交付前必须静态审核：

- **JSON 合法性**：拦截裸换行、未转义引号、UTF-8 BOM——这些都会让 MMD 静默导入失败。
- **双重转义**：回读 replaceString，解析后的 HTML 不应残留多余反斜杠。
- **平台红线**：旧版 MMD 检查 `<script>` 标签、ES6 语法、`innerHTML` 拼接、`style.cssText`。
- **字符数限额**：findRegex ≤ 1000、replaceString ≤ 20000、正则总数 ≤ 30。
- **规范字段**：chara_card_v2 规范（MMD 不识别 v3）、世界书蓝绿灯字段。

```bash
python skills/tavern-mmd/scripts/validate.py <你的导入json> --platform oldmmd
```

子代理可调用以节约主上下文。**任何 json 交付前还必须跑** `python -m json.tool <文件> > /dev/null`。

### 状态栏预览（`scripts/build-preview.py`）

生成自包含 HTML 沙箱，按平台模拟渲染限制（旧版 MMD 剥离 `<script>` 并裸露源码、ES6 标黄），主AI 用 Preview 工具看渲染、点按钮、切标签、测交互。

```bash
python skills/tavern-mmd/scripts/build-preview.py <你的导入json> --platform oldmmd
```

> ⚠️ **沙箱预览正常 ≠ MMD 正常**。沙箱用标准浏览器渲染，无法复现 MMD 的 markdown 管线（vditor）。典型踩坑：注入 HTML 标签间的换行/空行会被 MMD 解析成带 margin 的空 `<p>` 段落，渲染出大块横向空白条——浏览器把这些换行当可折叠空白，所以预览里完全看不出来，内容少的页（如酒吧页）实机导入后尤其刺眼。**状态栏 / 美化必须实机导入 MMD 验证**，沙箱只能拦其他类问题。防御办法见 `references/beautify/statusbar-radar.md` 的「MMD 换行空白条陷阱」一节（HTML 压成单行无缝 + `p:empty`/`p{margin:0}`/`br` 三态防御 CSS）。
...
| 产出 | 本地酒馆 | MMD（新旧同） |
|---|---|---|
| 角色卡 | chara_card_v3 json | chara_card_v2 json（MMD 仅识别 v2） |
