...
**MMD 两条路线（`/mmd`、`/mmdsandbox`）的字段级长度硬限**：

- `content` **≤3000 字**（`【官方文档】`字段长度表）。超了拆条 —— 一条塞满 3000 字本身就说明该分成几条。注意这与本 skill「单条 ≤800」的**成本软建议**是两件事：800 为控 token，3000 是不许越的线。
- `comment`（条目标题）**≤20 字**，判罚级别见第 5.2 节（当前 MMD ERROR / 沙盒 WARN）。
- 本地酒馆 `/st` 两项都无硬限。
