...
>
> B 组变量用于沙盒文档，不会自然跨源继承到宿主页，也不能替换平台图标。宿主另有 **21 个开放 data-host 根**的静态 `<style>` 抽取过滤通道；根表、禁区、summary/summary-confirm 范围见 [宿主换肤指南](sandbox-host-styles.md)。不是所有系统弹窗都开放，也没有新增跨源 DOM 或 SDK 接口。固定色值不称自动深浅适配。
...
>
> **范围限定**：2026-08 历史源码未发现对这些全局选择器的专项过滤，只描述 iframe 内样式，不能扩大成全部版本、全部 CSS“零过滤”。宿主转发另有根/嵌套/资源过滤，本地解析器的保守子集也不等于平台硬限制，见 [两条通路](sandbox-host-styles.md#3-两条-css-通路)。`:root` 选中 html，不能可靠覆盖后代平台 root 已定义的同名变量；换肤写 `[data-chat="root"][data-theme="dark"]`，并保持作者作用域，避免误改沙盒内原生组件。
