...
> 适用范围只有**当前 MMD**。旧版 MMD 的脚本能力和 CSP 不满足本协议；本地 SillyTavern 应走其原生主题或自定义 CSS。除非测试记录明确写明日期、版本和路径，本文件不宣称运行时资产已经通过 MMD 实机验证。
...
- 其他资产可能已修改同一属性。
- 旧版清污脚本可能删除过类、内联属性或文本，且没有 delta。
- 文本规范化属于不可逆内容改写。
