...
> **沙盒模式（`/mmdsandbox`）**：本文的**制作期**六维风格模型、规范 token、整体性检查与 light/dark 配对可以复用；当前 MMD 的 owner/route/native 运行时合同**不可复用**。沙盒产物由 SBK 编译器把六维 bundle 映射到平台 29 个 `--chat-*` 与自有 `--sbk-*`，主题状态只读平台 `data-theme="light|dark"` 并订 `theme:change`。作者不能新增主题状态 `data-*`（会被净化），也不使用 `:root/html/body` 作为 token 源。见 `sandbox-kit.md` 与 `../platforms/mmd-sandbox.md` §6。
