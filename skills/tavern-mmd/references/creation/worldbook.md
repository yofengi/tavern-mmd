...
- **MMD沙盒模式 `/mmdsandbox`**：15000 字固定传输上限**仍未确证**【待验证】；已确证的字段级上限是 `personality` 10000 字、`beginning` **4000 字**。在平台给出聊天页传输真值前，仍按 15000 预算控制蓝灯总量并留 2000–3000 字缓冲，但要明确这是保守制作预算，不冒充沙盒硬上限。
...
- **单条 `content` ≤ 3000 字**（`【官方文档】`字段长度表，MMD 两条路线共通）。这是字段级硬限，与下面「单条 ≤800」的质量软建议是两件事：800 是为控成本，3000 是不许越的线。
- **公开发布门槛**（只在要过公开审核时才卡，私人自用卡与纯草稿不受约束）：**固定传输字符 = 人设 + 「已启用 且 常驻 且 概率 100%」的世界书正文**，须落在 **2000–15000 含边界**；`beginning` **≥200 字**。关键词条目、停用条目、概率不足 100% 的常驻条目**不计入**；不许用空白换行凑数。注意这条的 15000 是**审核口径**，与沙盒那个「运行时截断预算 15000【待验证】」数值撞脸但性质不同。

**MMD 写卡留字符纪律：**

- 当前 MMD `/mmd`：固定传输字符 = 人设 + 性格 + 情境 + 全部蓝灯 content + 本轮命中的绿灯 content，硬上限 15000。
- 沙盒 `/mmdsandbox`：同类固定传输上限尚未确证；以下 15000 只作保守制作预算，不得写成平台硬上限。已确认的字段上限仍是 `personality <=10000`、`beginning <=4000`。

1. **蓝灯压瘦**：只放每轮都必须在的内容。当前 MMD 蓝灯直接吃 15000 额度；沙盒也会持续烧 token。
...
> **硬上限与软建议的关系**：
> - 当前 MMD：固定传输 `<=15000` 是硬上限。
> - 沙盒：15000 暂为保守预算，不能冒充已确认硬上限；字段级硬限按上文。
> - 单条 `<=800`、蓝灯 `<=5` 仍是质量/成本软建议。先守已确认硬限与预算，再优化质量。
