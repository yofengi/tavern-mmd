...
| `replaceString` 超10000字符截断 | 替换字符串被平台截断，后续内容丢失 | 使用分段式正则规则拆分；单条不超10000字符 |
...
| 限制 | 现象 / 后果 | 解决方案 |
|:---|:---|:---|
| HTML总长度约10000字符上限 | 超出部分被截断或整体失效 | 压缩代码，避免冗余；CSS用短类名 |
...
- 时刻注意 `replaceString` 总字符数，复杂 `random` 组合（尤其多个长选项）会快速消耗10000字符额度
...
- [ ] `findRegex` ≤ 1000字符，`replaceString` ≤ 10000字符，正则条目 ≤ 30条
