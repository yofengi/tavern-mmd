...
>
> **来源边界**：仍未确证的一律保留 `【待验证】` / `【原文未说明】`。当前已知空白：聊天页是否同施加 `Us` 长度截断（§8）· 15000 字固定传输在沙盒是否适用（§10.2）· `CACHE_QUOTA_BYTES` 超限行为（§4.4）· 取消订阅 API（§4）· 内联脚本是否对内容相同的多条去重（§2.4）· `SAFE_FOR_XML` 的无空格危险形态（§5.2）· **整个平台状态变量子系统的运行契约**（`sdk.vars` / `vars:change` 的签名与导入字段，官方 `contract.json` 至今未收录，§14）。
>
...
>
> 🚨 **并发调用拿 `BUSY`（`【官方文档】`）**：当前发送或回复生成还没结束（含平台自己的发送、重新生成）时再调 `send`，**立刻返回 `BUSY`** —— **不排队，也不消耗本次发送限频**。正确处理是**保留草稿、提示用户稍后自己发，不要自动重试**（重试只会连续拿 `BUSY`）。另外**不要在 `message:done` 里无条件再 `send`**，那是自问自答死循环（§11 排查表有对应项）。自带一个 `pending` 哨兵挡住连点最省事。
...
- **单条 `content` ≤ 3000 字**（`【官方文档】`字段长度表；本 skill 此前缺此项）。超了拆条 —— 一条塞 3000 字本身也说明该分成几条。
...
### 10.3 公开发布门槛（`【官方文档】`，只在要过公开审核时才卡）

官方 `validate.mjs --publish` 的算法，本 skill 此前完全缺失，补上：

**固定传输字符 = 人设 + 「已启用 且 常驻（`constant: true`） 且 概率 100%」的世界书条目正文**，合计必须落在 **2000–15000（含边界）**。

- **不计入**：关键词触发条目、已停用条目、概率不足 100% 的常驻条目。
- **`beginning` 至少 200 字**（这是**下限**，与 §8 那个 4000 上限是两头；夹在 200–4000 之间）。
- 🚨 **不许用空白/换行填充凑数**。
- **纯人设草稿、私人自用卡不受这条约束** —— 它只是公开发布的门槛，日常做卡别拿它当硬错误。

注意这里的 15000 与 §10.2 那个「给模型的输入预算 15000」数值撞脸但**性质不同**：这条是**审核口径**（官方明文），§10.2 那条是**运行时截断口径**（沙盒下仍 `【待验证】`）。别把两者混成一句话。
...
>
> 🚨 **不要做的三件事**：① 不要编造 `sdk.vars.get()` 的参数/返回结构；② 不要把 `varSchema` / `varInit` / 猜的变量条件字段塞进 6 键正则包（会让整包导入失败）；③ 不要因为契约缺失就对用户说「平台不支持状态变量」—— 它是**已确认存在但契约未公布**。

### 14.1 这是什么

平台自己的一套剧情状态存储，**仅新聊天页（`chatVersion: 1`）可选开启**，所以只在沙盒模式辖区内。作者在创卡侧配置状态结构、开局值和自然语言更新规则；平台负责把状态喂给模型、把快照随消息存档、按楼层回溯。

结构限制（`【官方文档】`）：节点支持**分组 / 数字 / 文字 / 是否**四种类型，**最多 6 层嵌套、80 个键**。没有状态节点的卡走普通流程，不受影响。

### 14.2 🚨 三种数据来源必须分清（写状态栏前先定这个）

| 来源 | 该放什么 | 与 AI / 历史的关系 |
|---|---|---|
| **平台状态变量** | 需要模型知道、且要跟着楼层回溯的剧情数据（好感、进度、关键 flag） | 本轮状态**进提示词**；完整快照随 AI 消息保存；回溯与重生成**按对应楼层恢复** |
| `sdk.save`（§4.5） | 作者小游戏进度、与 AI 无需同步的持久数据 | **不自动进 AI 上下文**，**不保证随剧情回溯** |
| `sdk.cache`（§4.4） | 展开/折叠状态、临时展示、当次页面数据 | 刷新即失，不作长期进度 |

🚨 **同一个剧情数值不要同时由平台状态表和 `save` 各存一份** —— 两份真值必然漂移，且回溯时只有一份会跟着回。传统 `[status]` 卡仍可继续从 AI 正文原文标记里解析（本文 §3 那条路照用），但要跟用户讲清：私有存档和历史楼层不是自动同步的。

### 14.3 `{{var}}` 宏的替换边界

`{{var}}` / `{{var.路径}}` **只在本轮新组装的**人设、世界书、设定补充、用户人设里替换，**不重写历史消息**。世界书条目的变量条件与原有关键词、概率等条件是**「并且」**关系（不是替代）。

功能栏（`statusbar` 字段）可以读状态宏 —— 但注意这跟 §7.5「功能栏是静态的」不冲突：宏在**组装时**替换一次，之后不会自己刷新。

### 14.4 🚨 `<abc_vars>` 内部块：绝对不要碰

平台统一注入内部 `<abc_vars>` 输出说明，自己负责截取、校验并合并部分状态树：**完整状态存在消息旁，干净剧情正文存进消息**。

三条红线：

1. **不要自己发明状态标签**去跟它抢（比如另造一套 `<vars>`）。
2. **不要用正则把 `<abc_vars>` 藏起来** —— 平台接口已经把它从正文里拆出去了，正文本来就是干净的；写规则去 hide 它只会打乱平台自己的解析。
3. 区分两类卡：**普通卡保存原始回复**；**平台状态卡**的内部块由接口拆出。做状态栏时先确认这张卡属于哪类，再决定是解析 AI 正文标记（§3）还是走状态变量。

### 14.5 HUD 侧怎么接（形态待验证）

官方口径是：HUD 通过 `sdk.vars.get()`、`sdk.vars.set()` 与 `vars:change` 事件更新。两条已明确的行为约束：

- **分享页和瘦预览可读不可写**（与 §4.11 瘦预览「写类能力不可用」一致，可交叉印证）。
- **状态更新只刷新值** —— 不要卸载重挂整个功能栏，也不要借机重跑整段脚本（§2 已说脚本整卡只跑一次）。

因为签名未公布，实际交付时的做法：

- **只讨论设定或产品规则** → 按上面已确认的行为回答，不碰代码。
- **要可运行的状态界面** → 先跟用户要新版 SDK 文档或导出样本；拿不到就交付已确定的部分（渲染层、协议层），把「状态读写待接入」写进交付说明。
- **能绕开就绕开**：现阶段做状态栏，走「AI 正文吐 `[状态]` 块 + 正则/脚本解析」（§3 + §7）这条路是**已确证可跑**的；状态变量是等契约补齐后的升级项，不是当下的必选项。

### 14.6 地图类持久化（官方给的唯一具体配方）
