...
⚠️ **不共通**：字段长度上限沙盒侧已由源码确证为另一套值（`findRegex` 真值 4096、`replaceString` 真值 100000、`scriptName` 真值 200、`beginning` 真值 4000），官方校验脚本那套数字**不是真值**，详见 §8。
...
- **瘦预览**：创卡页里那个预览是简化环境，改输入框、发送、存档大多不可用；**样式与舞台可用**。要验完整行为必须回聊天页。逐能力待遇见 §4.1 表格「瘦预览」列。
...
> 🚨 **用 JS 往功能栏 `appendChild` 的东西留不住。** 功能栏 HTML 由平台按 `statusbar` 字段**整块**写进去；创卡页里每改一次规则这块就重画一次（预览会重装整卡），`appendChild` 进去的东西会一起没。**改里面已有节点的文字、class、内联 style 没问题** —— 这正是做动态状态栏的正路：**骨架写进规则（保证它一定存在），值靠 JS 改**。长期面板放舞台。
...
| `scriptName` | 名称 | 非空。真上限 **200** 字（`【源码确证】`），官方校验卡 20 → 实践仍按 20 写，见 §8 |
| `findRegex` | 匹配式 | 非空。真上限 **4096** 字（`【源码确证】`），官方校验卡 1000 → 见 §8 |
| `replaceString` | 替换内容 | 真上限 **100000** 字（`【源码确证】`），编辑器 UI 显示 20000 → 见 §8 |
...
| `beginning` | **4000 字**（源码真值，🚨 官方校验脚本的 10240 是错的） | 超出**静默截断**；见 §8.2 |
| `personality` | **10000 字** | 官方 ERROR（公开卡审核文案建议 2000–5000） |
| `scriptName` | 真值 **200 字** / 实践按 **20 字** | 官方校验偏严；见 §8.2 |
| `findRegex` | 真值 **4096 字** / 实践按 **1000 字** | 官方校验偏严；见 §8.2 |
| `replaceString` | 真值 **100000 字** / 实践按 **20000 字** | 编辑器 UI 卡 20000；见 §8.2 |
...
### 8.2 🚨 真实长度上限：源码常量推翻官方校验脚本

`【源码确证】`沙盒源码里的常量与截断函数，逐字：

```js
var Us = { beginning:4e3, statusbar:200, imageUrl:2048, name:200, regex:4096, content:1e5, regexList:130 };
function Ws(e,t){ return typeof e===`string` ? (e.length>t ? e.slice(0,t) : e) : `` }   // 静默截断
```

`Ws()` 是**静默 slice** —— 超限**不报错、不提示，直接砍掉后半段**。创卡页 UI 的实机计数器显示 `7/4000`，印证 `beginning` 真值是 4000。

| 字段 | **源码真值** | 官方 skill 校验脚本 | 判定 |
|---|---|---|---|
| `beginning` | **4000** | 10240 | 🚨 **校验脚本错**（创卡页 UI 也显示 4000）。按 10240 写会被静默截断 |
| `scriptName` / `name` | **200** | 20 | 校验脚本**偏严** |
| `findRegex` / `regex` | **4096** | 1000 | 校验脚本**偏严** |
| `replaceString` / `content` | **100000** | 20000 | 校验脚本**偏严**（编辑器 UI 显示 /20000） |
| `statusbar` | 200 | 200 | ✅ 一致 |
| `regexList` | 130 | 130 | ✅ 一致 |
| `imageUrl` | **2048** | 未记录 | 新增 |

> ⚠️ **`【待验证】`：该常量位于「创卡预览草稿校验路径」（`Ks` / `Ws`）。聊天页吃的是服务端数据，是否施加同一套截断未能确证。** 所以下面的取值策略是**双轨的**，不是「真值更宽所以随便写」。

**本 skill 的取值策略**：

- **ERROR 阈值取真值（宽）** —— 校验脚本不该拿一个比平台更严的数字去阻断交付。特别是 `beginning`：**必须改成 4000 卡 ERROR**，因为按 10240 写是真的会丢内容。
- **实践建议取编辑器 UI 值（严）** —— `scriptName` ≤ 20、`findRegex` ≤ 1000、`replaceString` ≤ 20000。理由很实在：**超了作者一进编辑器就被截断**，卡片在他手里当场坏掉，而他看不到任何提示。
- 遇到确实必须超限的场景（比如一段复杂 `<script>` 压不进 20000），可以放宽到真值，但**必须在交付说明里写明「不要在创卡页编辑器里打开这条规则，否则会被截断」**。

原文这里写的「`replaceString` 的 20000 是编辑器上限而非导入上限」方向是对的，现在有了确切数字：**导入/运行时真上限 100000，编辑器 UI 截断线 20000**。
...
9. **验证顺序**：创卡页瘦预览先看样式与舞台（注意 `save.*` 会抛、预览会重跑）→ 回聊天页加 `?sdkDebug=1` 验输入框/发送/存档 → 按 §11 排查表定位。
