...
| 用户消息 | 复制 / 删除 / 回溯 —— **3 项，无「开启新的故事」**（2026-09-06 卡作者口述订正；此前写成「同 AI 四项」是把方向填反了） |
| 角色卡「第一句话」 | **仅**「复制」 |
...
上表三种菜单的**项数**均来自卡作者口述（2026-08-29 与 2026-09-06 两次，后者明确区分了
AI 与玩家两套）+ `message-menu` CSS 佐证。各项**真实副作用**（尤其回溯的删除范围）与
菜单文案逐字，**未在真机逐一点击复核**，仍标 `probe-needed`。

🚨 2026-09-06 订正：此前这里把用户菜单写成「同 AI 四项」。作者两次口述都是**玩家菜单 3 项**、
没有「开启新的故事」——当时标了 probe-needed 却按"同 AI"猜着填，把方向填反了。预览侧已改为 3 项并加断言防回退。教训：标了 probe-needed 的格子应当**留空或写明未知**，
不要用"大概同某某"填上——填进去之后它看起来就和实测项一样可信了。
