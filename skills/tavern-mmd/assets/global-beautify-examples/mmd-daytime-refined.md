...
> **历史参考，非默认成品**：本文件保留的是 **2026-06-21 当前 MMD** 页面选择器与米白/酒红主题案例，**仅适用于当前 MMD（`/mmd`）**，不适用于本地 SillyTavern。它来自用户提供的社区快照，作者、原 URL 与许可证未记录；skill 资产库仅作历史收录与最低限度债务修复，不宣称原创，也没有在本次整理中重新宣称 MMD 实机验证。
>
> 🚨 **不适用于 MMD沙盒模式（`/mmdsandbox`）**：本文的选择器是当时抓的平台内部 class 名，而沙盒模式明确「平台自己的 class 名会变，不要抄页面上看到的 `.xxx`」，只承诺 `[data-chat]` / `[data-slot]` 不改名。本文把样式写到 `:root,body`，这类全局选择器在沙盒模式是官方校验 WARN（须改写成 `[data-chat="root"]`）；激活载体 `img onerror` 也被官方明令禁止。沙盒模式换肤见 `../../references/beautify/global-css.md`「沙盒模式换肤」。
>
