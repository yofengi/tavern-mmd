...
| ES6+ 语法 | ✅ | ✅ 实测全支持（img onerror 载体下，7/7 语法探针全绿），**推荐 ES6** | ✅ 官方示例即用 `const` 等 ES6 写法 |
| 正则导入方式 | json 直接导入 | json导入（MMD专用4字段格式）或UI手填 | 创卡页「导入正则」json（**6键格式**，多 `chatVersion`/`personality`）或UI手填；**不能导入整卡** |
| 正则限额 | 无硬限制 | ≤130条；findRegex≤1000字符；replaceString≤20000字符 | ≤130条；findRegex≤1000；replaceString≤20000（编辑器上限，超了拆条）；另 statusbar 200 / beginning 10240 / personality 10000 / scriptName 20 |
...
**沙盒模式证据等级**：结论来自官方手册与官方校验脚本（「官方明文」级），本 skill **尚未做过实机探针**，不等同于 `/mmd` 那些「已实测」边界。官方未说明的（沙盒底层形态、`document.currentScript`）在平台文档里标 `【原文未说明】`，不得据此写取巧代码。另有一条真红线：`chatVersion` **只在新建卡导入时被读取**，无法通过导入把老卡升级成新页 —— 交付时必须书面提醒用户新建卡。
...
当前 MMD 独立正则导入 JSON 的顶层必须恰好且仅有 `pageDepth/statusbar/beginning/regex_scripts` 四键；沙盒模式恰好且仅有上表那六键（`chatVersion` 必须为 `1`）。两者每条 `regex_scripts` 规则都必须恰好且仅有 `id/scriptName/findRegex/replaceString` 四键——但沙盒模式的 `id` 必须是负数，且 `findRegex` 不强制 slash literal（纯字面量标记是首选）。当前 MMD 的 `findRegex` 仍必须写成 `/pattern/flags` slash literal，固定标记也要包斜杠。所有 json 交付前必须语法校验：`python -m json.tool <文件> > /dev/null`；再跑 `python scripts/validate.py <文件> --platform <mmd|mmdsandbox|st>`（`--platform` 默认 `mmd`）。
