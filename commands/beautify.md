...
4. 按类型读取文档并制作：
   ...
   - 状态栏（`/mmdsandbox`）：**直接用 SBK 基座**，别从零手写。读 `references/beautify/sandbox-kit.md` 选双模（A 常驻 HUD / B 消息内快照，可并存），再改 `assets/sandbox-kit/sbk.config.example.json` 的 `schema.fields` 跑 `build_sbk.py`。字段与布局风格照样先弹窗问。三条根本约束：**雷达法与 img onerror 引擎不可移植**（官方禁 onerror）；**功能栏静态**（装载时只跑一次）→ 动态更新只能靠 JS 改 DOM；**`ready` 最后到且无补发** → 首屏挂 `message:mount`/`message:done`。平台细节见 references/platforms/mmd-sandbox.md（§3 占位陷阱、§7 正则、§12 写作策略）。
