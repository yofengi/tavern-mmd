...
2.1. **全局美化分流（先按平台分，再问档位）**：
   ...
   - `/mmdsandbox`：**不走上面二档**。沙盒模式有平台自己的 10 个 `--chat-*` 变量、`theme:change` 事件、舞台与服务端存档，是另一套全局美化故事；换肤在 `[data-chat="root"]` 上改变量，禁 `*`/`html`/`body`/`:root` 全局选择器，常驻面板挂 `sdk.stage` 而不是气泡。**先读 `references/platforms/mmd-sandbox.md`（§6 CSS、§4.5-4.6 存档与舞台）**，不要移植当前 MMD 的 runtime 主题包。
...
4. 按类型读取文档并制作：
   ...
   - 状态栏（`/mmdsandbox`）：**雷达法与 img onerror 引擎不可移植**（官方禁 onerror 点火器）。改走「专开一条只放 `<script>` 的规则」+ `message:done` 取 `msg.content` 解析后渲染；短小固定块可纯规则替换（`$1`/`$名字`）零 JS；常驻面板挂舞台。字段与布局风格照样先弹窗问，技术细节全看 references/platforms/mmd-sandbox.md（§3 占位陷阱、§7 正则、§12 写作策略）。
...
5. 代码草稿放"工作/"；每条正则统计字符数（MMD 限额预检，replaceString 达到 18000 即预警）。**当前 MMD** 每条 findRegex 必须是 `/pattern/flags` slash literal；**沙盒模式**不强制，纯字面量标记（`{{hud}}`）是首选，且 `id` 必须为负数。
