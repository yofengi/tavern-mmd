...
4. 向用户输出一句确认 + 该平台关键限制摘要（5-7行）：
   ...
   - **交付两条路线，做整卡时用弹窗问一次**（`references/output/card-json.md` §8.1）：**路线A** = v2 整卡 PNG/JSON（世界书进 `character_book`、正则进 `data.extensions.regex_scripts`、人设进 `data.description`，**无手工粘贴**）；**路线B** = 6键正则 json + persona 文本（导入页不读 `personality`，须手工粘贴）+（可选）独立世界书 json。沙盒**能导 v2 整卡**（`【用户实测】`编辑页导入按新卡处理），旧版「不走 chara_card_v2、不产整卡 PNG」已更正。两条路线都要提醒「**新建卡 + 首次保存前在创卡页选「使用新版」聊天页**，此选择首次保存后永久不可改」。
   ...
   - **平台状态变量子系统 `sdk.vars` / `vars:change` / `<abc_vars>`（§14）**：业务规则已确认（仅新页可开、最多 6 层 80 键、状态快照随消息存并按楼层回溯、`{{var}}` 只替换本轮新组装内容不改历史、分享页与瘦预览可读不可写），但**运行契约官方至今未公布**（`contract.json` 无这几个名字）→ 一律 `【待验证】`，**不许编造签名**，也不许把 `varSchema`/`varInit` 塞进 6 键正则包；同时**不要**对用户说「平台不支持」。现阶段做状态栏仍走「AI 正文吐 `[状态]` 块 + 脚本解析」这条已确证的路。`<abc_vars>` 是平台内部块，**不要自造状态标签跟它抢，也不要用正则藏它**。
